...
t_max = 6  # total simulation duration in seconds
# Set initial state
...
ALTERNATIVE_CONTROL_POINT = False
...
def compute_control_input(desired_state, robot_state):
    # initial numpy array for [vlin, omega, wl, wr, ux, uy]
    all_input = np.array([0., 0., 0., 0., 0., 0.])
    ...
    if not ALTERNATIVE_CONTROL_POINT:
        v = Kv * distance
        w = Kw * angle
        all_input[4:6] = Kv * (desired_state[0:2] - robot_state[0:2])
    else:
        ...
        # POI control input
        u_gtg = Kv * (desired_state[0:2] - robot_state[0:2])
        all_input[4:6] = u_gtg
...
def simulate_control():
    ...
    input_history = np.zeros((sim_iter, 6))  # for [vlin, omega, wl, wr, ux, uy] vs iteration time
...
if __name__ == '__main__':
    ...
    ax.plot(t, input_history[:, 4], label='ux [m/s]')
    ax.plot(t, input_history[:, 5], label='uy [m/s]')
